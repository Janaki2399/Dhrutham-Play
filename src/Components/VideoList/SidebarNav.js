    ...
    return(
        <nav>
            {list.map((item,index)=>
                    ...
                    <div className="margin-right"><img src={`https://img.youtube.com/vi/${item.videoId}/0.jpg`} alt="img"
                           width="120" /></div>
                    <div className="flex-horizontal space-between center-align">
                    <div className="font-bold-1 font-size-5">{getVideoObject(item.videoId).name}</div> 
                    {isUserPlayList&&<RemoveButton id={item.videoId} removeItem={removeItem}/>}
                    </div>
